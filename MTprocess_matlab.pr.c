/* Process model C form file: MTprocess_matlab.pr.c */
/* Portions of this file copyright 1986-2008 by OPNET Technologies, Inc. */



/* This variable carries the header into the object file */
const char MTprocess_matlab_pr_c [] = "MIL_3_Tfile_Hdr_ 145A 30A op_runsim 7 641C30F5 641C30F5 1 LAPTOP-45T4L382 12085 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 1bcc 1                                                                                                                                                                                                                                                                                                                                                                                                  ";
#include <string.h>
...
#define FIN_PREAMBLE_DEC	MTprocess_matlab_state *op_sv_ptr;
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((MTprocess_matlab_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
extern "C" {
#endif
	void MTprocess_matlab (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Obtype _op_MTprocess_matlab_init (int * init_block_ptr);
	void _op_MTprocess_matlab_diag (OP_SIM_CONTEXT_ARG_OPT);
	void _op_MTprocess_matlab_terminate (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Address _op_MTprocess_matlab_alloc (VosT_Obtype, int);
	void _op_MTprocess_matlab_svar (void *, const char *, void **);
...
void
MTprocess_matlab (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	FIN_MT (MTprocess_matlab ());
		...
		FSM_ENTER ("MTprocess_matlab")

		FSM_BLOCK_SWITCH
			{
			/*---------------------------------------------------------*/
			/** state (Init) enter executives **/
			FSM_STATE_ENTER_FORCED_NOLABEL (0, "Init", "MTprocess_matlab [Init enter execs]")
				FSM_PROFILE_SECTION_IN ("MTprocess_matlab [Init enter execs]", state0_enter_exec)
				{
				//静态拓扑初始化
			...
			FSM_STATE_EXIT_FORCED (0, "Init", "MTprocess_matlab [Init exit execs]")


			/** state (Init) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "Init", "Idle", "tr_0", "MTprocess_matlab [Init -> Idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (Idle) enter executives **/
			FSM_STATE_ENTER_UNFORCED (1, "Idle", state1_enter_exec, "MTprocess_matlab [Idle enter execs]")

			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (3,"MTprocess_matlab")


			/** state (Idle) exit executives **/
			FSM_STATE_EXIT_UNFORCED (1, "Idle", "MTprocess_matlab [Idle exit execs]")
				FSM_PROFILE_SECTION_IN ("MTprocess_matlab [Idle exit execs]", state1_exit_exec)
				{
				//frame_type=0;
			...
			FSM_PROFILE_SECTION_IN ("MTprocess_matlab [Idle trans conditions]", state1_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 4, state4_enter_exec, ;, "sim_end", "", "Idle", "Result", "tr_1", "MTprocess_matlab [Idle -> Result : sim_end / ]")
				FSM_CASE_TRANSIT (1, 2, state2_enter_exec, ;, "frame_arrive", "", "Idle", "Receive", "tr_16", "MTprocess_matlab [Idle -> Receive : frame_arrive / ]")
				FSM_CASE_TRANSIT (2, 3, state3_enter_exec, ;, "send_data", "", "Idle", "Send_data", "tr_18", "MTprocess_matlab [Idle -> Send_data : send_data / ]")
				FSM_CASE_TRANSIT (3, 5, state5_enter_exec, ;, "send_state_frame", "", "Idle", "Send_state", "tr_19", "MTprocess_matlab [Idle -> Send_state : send_state_frame / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_FORCED (2, "Receive", state2_enter_exec, "MTprocess_matlab [Receive enter execs]")
				FSM_PROFILE_SECTION_IN ("MTprocess_matlab [Receive enter execs]", state2_enter_exec)
				{
				timing=op_sim_time();
			...
			FSM_STATE_EXIT_FORCED (2, "Receive", "MTprocess_matlab [Receive exit execs]")


			/** state (Receive) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "Receive", "Idle", "tr_15", "MTprocess_matlab [Receive -> Idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (Send_data) enter executives **/
			FSM_STATE_ENTER_FORCED (3, "Send_data", state3_enter_exec, "MTprocess_matlab [Send_data enter execs]")
				FSM_PROFILE_SECTION_IN ("MTprocess_matlab [Send_data enter execs]", state3_enter_exec)
				{
				timing=op_sim_time();
			...
			FSM_STATE_EXIT_FORCED (3, "Send_data", "MTprocess_matlab [Send_data exit execs]")


			/** state (Send_data) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "Send_data", "Idle", "tr_17", "MTprocess_matlab [Send_data -> Idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (Result) enter executives **/
			FSM_STATE_ENTER_UNFORCED (4, "Result", state4_enter_exec, "MTprocess_matlab [Result enter execs]")
				FSM_PROFILE_SECTION_IN ("MTprocess_matlab [Result enter execs]", state4_enter_exec)
				{
				/*
				if(self_adress==1)
					{
				...
				*/
				...
				engEvalString(ep, "b = 2;");
				engEvalString(ep, "result = a + b;");
				
				
				result = engGetVariable(ep, "result");
				printf("*****************************The result variable is: %f\n", result);
				ptr = mxGetPr(result);
				printf("*****************************The ptr variable is: %f\n", ptr);
				res = ptr[0];
				
				
				printf("*****************************The matlab result is: %f\n", res);
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (9,"MTprocess_matlab")


			/** state (Result) exit executives **/
			FSM_STATE_EXIT_UNFORCED (4, "Result", "MTprocess_matlab [Result exit execs]")
			...
			FSM_STATE_ENTER_FORCED (5, "Send_state", state5_enter_exec, "MTprocess_matlab [Send_state enter execs]")
				FSM_PROFILE_SECTION_IN ("MTprocess_matlab [Send_state enter execs]", state5_enter_exec)
				{
				timing=op_sim_time();
			...
			FSM_STATE_EXIT_FORCED (5, "Send_state", "MTprocess_matlab [Send_state exit execs]")


			/** state (Send_state) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "Send_state", "Idle", "tr_20", "MTprocess_matlab [Send_state -> Idle : default / ]")
				/*---------------------------------------------------------*/
...
void
_op_MTprocess_matlab_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	/* No Diagnostic Block */
	}
...
void
_op_MTprocess_matlab_terminate (OP_SIM_CONTEXT_ARG_OPT)
	{

	FIN_MT (_op_MTprocess_matlab_terminate ())
...
/* local variable prs_ptr in _op_MTprocess_matlab_svar function. */
...
VosT_Obtype
_op_MTprocess_matlab_init (int * init_block_ptr)
	{
	VosT_Obtype obtype = OPC_NIL;
	FIN_MT (_op_MTprocess_matlab_init (init_block_ptr))

	obtype = Vos_Define_Object_Prstate ("proc state vars (MTprocess_matlab)",
		sizeof (MTprocess_matlab_state));
	*init_block_ptr = 0;
...
VosT_Address
_op_MTprocess_matlab_alloc (VosT_Obtype obtype, int init_block)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	MTprocess_matlab_state * ptr;
	FIN_MT (_op_MTprocess_matlab_alloc (obtype))

	ptr = (MTprocess_matlab_state *)Vos_Alloc_Object (obtype);
	if (ptr != OPC_NIL)
		{
		ptr->_op_current_block = init_block;
#if defined (OPD_ALLOW_ODB)
		ptr->_op_current_state = "MTprocess_matlab [Init enter execs]";
#endif
		}
...
void
_op_MTprocess_matlab_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	MTprocess_matlab_state		*prs_ptr;

	FIN_MT (_op_MTprocess_matlab_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (MTprocess_matlab_state *)gen_ptr;
