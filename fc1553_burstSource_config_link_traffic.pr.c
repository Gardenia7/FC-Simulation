/* Process model C form file: fc1553_burstSource_config_link_traffic.pr.c */
/* Portions of this file copyright 1986-2008 by OPNET Technologies, Inc. */



/* This variable carries the header into the object file */
const char fc1553_burstSource_config_link_traffic_pr_c [] = "MIL_3_Tfile_Hdr_ 145A 30A op_runsim 7 64015978 64015978 1 LAPTOP-45T4L382 12085 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 1bcc 1                                                                                                                                                                                                                                                                                                                                                                                                  ";
#include <string.h>
...
typedef struct
	...
	int	                    		self_adress                                     ;
	double	                 		burst_traffic_link_ratio_sum[32]                ;
	} fc1553_burstSource_config_link_traffic_state;
...
#define uniform_dist_01         		op_sv_ptr->uniform_dist_01
...
#define burst_traffic_link_ratio_sum		op_sv_ptr->burst_traffic_link_ratio_sum
...
#define FIN_PREAMBLE_DEC	fc1553_burstSource_config_link_traffic_state *op_sv_ptr;
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((fc1553_burstSource_config_link_traffic_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
static void
	...
	double              burst_traffic_link_ratio[32];
	char                *burst_traffic_link[]={"NC","NT1","NT2","NT3","NT4","NT5","NT6","NT7","NT8","NT9","NT10","NT11","NT12","NT13","NT14","NT15","NT16","NT17","NT18","NT19","NT20","NT21","NT22","NT23","NT24","NT25","NT26","NT27","NT28","NT29","NT30","NT31"};
//	OmsT_Dist_Handle	start_time_dist_handle;
	...
	op_ima_obj_attr_get (op_topo_parent(op_id_self ()), "EN Node",            &EN_burst  );//NT节点使能属性

	
	//读取节点中关于节点各连接的流量分配的参数,用于确定节点的各个连接的流量占节点流量的比例
	op_ima_obj_attr_get (op_topo_parent(op_id_self ()), "Burst Traffic Link Parameters", &traf_gen_comp_attr_objid);//读取节点中关于节点各连接的流量分配的参数
	traf_conf_objid = op_topo_child (traf_gen_comp_attr_objid, OPC_OBJTYPE_GENERIC, 0);
	for(i=0;i<node_num;i++)
		{
		if(i==self_adress)
			burst_traffic_link_ratio[i]=0;
		else
			op_ima_obj_attr_get (traf_conf_objid,burst_traffic_link[i],&burst_traffic_link_ratio[i]);
		}
	for(i=0;i<node_num;i++)
		{
		
		Burst_traffic_control_matrix[i][self_adress]=burst_traffic_link_ratio[i];
		//if(self_adress==1)
			//printf("burst_traffic_link_ratio[i]:%f\n",burst_traffic_link_ratio[i]);
		}
	...
	for(i=0;i<node_num;i++)
		{
		if(i==0)
			burst_traffic_link_ratio_sum[i]=burst_traffic_link_ratio[i];
		else
			burst_traffic_link_ratio_sum[i]=burst_traffic_link_ratio[i]+burst_traffic_link_ratio_sum[i-1];
		
		//if(self_adress==0)
			//printf("burst_traffic_link_ratio_sum[i]:%f--------------------\n",burst_traffic_link_ratio_sum[i]);
		}
	...
	if ((start_time >= 0.0) && (stop_time > start_time)) 
		...
		uniform_dist_01=op_dist_load("uniform_int",1,10000);//产生一个0到1的均匀分布
...
extern "C" {
#endif
	void fc1553_burstSource_config_link_traffic (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Obtype _op_fc1553_burstSource_config_link_traffic_init (int * init_block_ptr);
	void _op_fc1553_burstSource_config_link_traffic_diag (OP_SIM_CONTEXT_ARG_OPT);
	void _op_fc1553_burstSource_config_link_traffic_terminate (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Address _op_fc1553_burstSource_config_link_traffic_alloc (VosT_Obtype, int);
	void _op_fc1553_burstSource_config_link_traffic_svar (void *, const char *, void **);
...
void
fc1553_burstSource_config_link_traffic (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	FIN_MT (fc1553_burstSource_config_link_traffic ());
		...
		FSM_ENTER ("fc1553_burstSource_config_link_traffic")

		FSM_BLOCK_SWITCH
			{
			/*---------------------------------------------------------*/
			/** state (init) enter executives **/
			FSM_STATE_ENTER_UNFORCED_NOLABEL (0, "init", "fc1553_burstSource_config_link_traffic [init enter execs]")
				FSM_PROFILE_SECTION_IN ("fc1553_burstSource_config_link_traffic [init enter execs]", state0_enter_exec)
				{
			...
			FSM_EXIT (1,"fc1553_burstSource_config_link_traffic")


			/** state (init) exit executives **/
			FSM_STATE_EXIT_UNFORCED (0, "init", "fc1553_burstSource_config_link_traffic [init exit execs]")


			/** state (init) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "init", "off", "tr_0", "fc1553_burstSource_config_link_traffic [init -> off : default / ]")
				/*---------------------------------------------------------*/



			/** state (off) enter executives **/
			FSM_STATE_ENTER_UNFORCED (1, "off", state1_enter_exec, "fc1553_burstSource_config_link_traffic [off enter execs]")
				FSM_PROFILE_SECTION_IN ("fc1553_burstSource_config_link_traffic [off enter execs]", state1_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (3,"fc1553_burstSource_config_link_traffic")


			/** state (off) exit executives **/
			FSM_STATE_EXIT_UNFORCED (1, "off", "fc1553_burstSource_config_link_traffic [off exit execs]")
				FSM_PROFILE_SECTION_IN ("fc1553_burstSource_config_link_traffic [off exit execs]", state1_exit_exec)
				{
			...
			FSM_TRANSIT_ONLY ((INACTIVE_TO_ACTIVE), 2, state2_enter_exec, ;, off, "INACTIVE_TO_ACTIVE", "", "off", "on", "tr_1", "fc1553_burstSource_config_link_traffic [off -> on : INACTIVE_TO_ACTIVE / ]")
				/*---------------------------------------------------------*/



			/** state (on) enter executives **/
			FSM_STATE_ENTER_UNFORCED (2, "on", state2_enter_exec, "fc1553_burstSource_config_link_traffic [on enter execs]")
				FSM_PROFILE_SECTION_IN ("fc1553_burstSource_config_link_traffic [on enter execs]", state2_enter_exec)
				{
				int destination_get_success=0;
				int i;
				time_test=op_sim_time();
				//printf("self_adress12212:%d--------\n",self_adress);
				if (op_intrpt_code () == OFF_TO_ON)
					...
					while(destination_get_success==0)
						{
						
						Probability = op_dist_outcome  (uniform_dist_01)/10000.0;
						...
						for(i=0;i<node_num;i++)
							...
							if(i==0)
								{
									if(Probability>0 && Probability<burst_traffic_link_ratio_sum[i])
										{
										Burst_destination=i;
										destination_get_success=1;	
										//printf("222222222******Burst_destination:%d********\n",Burst_destination);
										//printf("Probability:%f******Burst_destination:%d*****burst_traffic_link_ratio_sum[i]:%f******\n",Probability,Burst_destination,burst_traffic_link_ratio_sum[i]);
									
										}
								}
							else
								{
									if(Probability>burst_traffic_link_ratio_sum[i-1] && Probability<burst_traffic_link_ratio_sum[i])
										{
										Burst_destination=i;
										destination_get_success=1;
										////printf("222222222******Burst_destination:%d********\n",Burst_destination);
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (5,"fc1553_burstSource_config_link_traffic")


			/** state (on) exit executives **/
			FSM_STATE_EXIT_UNFORCED (2, "on", "fc1553_burstSource_config_link_traffic [on exit execs]")
				FSM_PROFILE_SECTION_IN ("fc1553_burstSource_config_link_traffic [on exit execs]", state2_exit_exec)
				{
				/* Determine the type of interrupt.		*/
				intrpt_type = op_intrpt_type ();
				intrpt_code = op_intrpt_code ();
			...
			FSM_PROFILE_SECTION_IN ("fc1553_burstSource_config_link_traffic [on trans conditions]", state2_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 1, state1_enter_exec, ;, "ACTIVE_TO_INACTIVE", "", "on", "off", "tr_3", "fc1553_burstSource_config_link_traffic [on -> off : ACTIVE_TO_INACTIVE / ]")
				FSM_CASE_TRANSIT (1, 2, state2_enter_exec, ;, "REMAIN_ACTIVE", "", "on", "on", "tr_4", "fc1553_burstSource_config_link_traffic [on -> on : REMAIN_ACTIVE / ]")
				}
				/*---------------------------------------------------------*/
		...
		FSM_EXIT (0,"fc1553_burstSource_config_link_traffic")
		}
	}
...
void
_op_fc1553_burstSource_config_link_traffic_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	/* No Diagnostic Block */
	}
...
void
_op_fc1553_burstSource_config_link_traffic_terminate (OP_SIM_CONTEXT_ARG_OPT)
	{

	FIN_MT (_op_fc1553_burstSource_config_link_traffic_terminate ())
...
/* local variable prs_ptr in _op_fc1553_burstSource_config_link_traffic_svar function. */
#undef pid_string
#undef debug_mode
#undef stop_time
...
#undef self_adress
#undef burst_traffic_link_ratio_sum
...
VosT_Obtype
_op_fc1553_burstSource_config_link_traffic_init (int * init_block_ptr)
	{
	VosT_Obtype obtype = OPC_NIL;
	FIN_MT (_op_fc1553_burstSource_config_link_traffic_init (init_block_ptr))

	obtype = Vos_Define_Object_Prstate ("proc state vars (fc1553_burstSource_config_link_traffic)",
		sizeof (fc1553_burstSource_config_link_traffic_state));
	*init_block_ptr = 0;
...
VosT_Address
_op_fc1553_burstSource_config_link_traffic_alloc (VosT_Obtype obtype, int init_block)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	fc1553_burstSource_config_link_traffic_state * ptr;
	FIN_MT (_op_fc1553_burstSource_config_link_traffic_alloc (obtype))

	ptr = (fc1553_burstSource_config_link_traffic_state *)Vos_Alloc_Object (obtype);
	if (ptr != OPC_NIL)
		{
		ptr->_op_current_block = init_block;
#if defined (OPD_ALLOW_ODB)
		ptr->_op_current_state = "fc1553_burstSource_config_link_traffic [init enter execs]";
#endif
		}
...
void
_op_fc1553_burstSource_config_link_traffic_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	fc1553_burstSource_config_link_traffic_state		*prs_ptr;

	FIN_MT (_op_fc1553_burstSource_config_link_traffic_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (fc1553_burstSource_config_link_traffic_state *)gen_ptr;
	...
	if (strcmp ("uniform_dist_01" , var_name) == 0)
		{
		*var_p_ptr = (void *) (&prs_ptr->uniform_dist_01);
		FOUT
		}
	...
	if (strcmp ("burst_traffic_link_ratio_sum" , var_name) == 0)
		{
		*var_p_ptr = (void *) (prs_ptr->burst_traffic_link_ratio_sum);
		FOUT
		}
	*var_p_ptr = (void *)OPC_NIL;
