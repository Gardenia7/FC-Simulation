/* Process model C form file: fc1553_periodSource_NT.pr.c */
/* Portions of this file copyright 1986-2008 by OPNET Technologies, Inc. */



/* This variable carries the header into the object file */
const char fc1553_periodSource_NT_pr_c [] = "MIL_3_Tfile_Hdr_ 145A 30A op_runsim 7 64015978 64015978 1 LAPTOP-45T4L382 12085 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 1bcc 1                                                                                                                                                                                                                                                                                                                                                                                                  ";
#include <string.h>
...
#define FIN_PREAMBLE_DEC	fc1553_periodSource_NT_state *op_sv_ptr;
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((fc1553_periodSource_NT_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
extern "C" {
#endif
	void fc1553_periodSource_NT (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Obtype _op_fc1553_periodSource_NT_init (int * init_block_ptr);
	void _op_fc1553_periodSource_NT_diag (OP_SIM_CONTEXT_ARG_OPT);
	void _op_fc1553_periodSource_NT_terminate (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Address _op_fc1553_periodSource_NT_alloc (VosT_Obtype, int);
	void _op_fc1553_periodSource_NT_svar (void *, const char *, void **);
...
void
fc1553_periodSource_NT (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	FIN_MT (fc1553_periodSource_NT ());
		...
		FSM_ENTER ("fc1553_periodSource_NT")

		FSM_BLOCK_SWITCH
			{
			/*---------------------------------------------------------*/
			/** state (init) enter executives **/
			FSM_STATE_ENTER_UNFORCED_NOLABEL (0, "init", "fc1553_periodSource_NT [init enter execs]")
				FSM_PROFILE_SECTION_IN ("fc1553_periodSource_NT [init enter execs]", state0_enter_exec)
				{
				...
				op_ima_obj_attr_get (op_topo_parent(op_id_self ()),"Period Enable",&EN_period);
				...
				period_request_map[1][self_adress-1] = EN_period;//周期性业请求列表初始化
				...
				/*判断当前这个数据源是否使能，使能才周期性的产生数据，否则直到仿真结束均不能产生数据		change by wj 2015-6-23	*/
			...
			FSM_EXIT (1,"fc1553_periodSource_NT")


			/** state (init) exit executives **/
			FSM_STATE_EXIT_UNFORCED (0, "init", "fc1553_periodSource_NT [init exit execs]")
				FSM_PROFILE_SECTION_IN ("fc1553_periodSource_NT [init exit execs]", state0_exit_exec)
				{
			...
			FSM_PROFILE_SECTION_IN ("fc1553_periodSource_NT [init trans conditions]", state0_trans_conds)
			FSM_INIT_COND (START)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 1, state1_enter_exec, ss_packet_generate();, "START", "ss_packet_generate()", "init", "generate", "tr_0", "fc1553_periodSource_NT [init -> generate : START / ss_packet_generate()]")
				FSM_CASE_TRANSIT (1, 2, state2_enter_exec, ;, "DISABLED", "", "init", "stop", "tr_1", "fc1553_periodSource_NT [init -> stop : DISABLED / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (1, "generate", state1_enter_exec, "fc1553_periodSource_NT [generate enter execs]")
				FSM_PROFILE_SECTION_IN ("fc1553_periodSource_NT [generate enter execs]", state1_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (3,"fc1553_periodSource_NT")


			/** state (generate) exit executives **/
			FSM_STATE_EXIT_UNFORCED (1, "generate", "fc1553_periodSource_NT [generate exit execs]")
				FSM_PROFILE_SECTION_IN ("fc1553_periodSource_NT [generate exit execs]", state1_exit_exec)
				{
				/* Determine the code of the interrupt, which is used in evaluating	*/
				/* state transition conditions.										*/
				intrpt_code = op_intrpt_code ();
			...
			FSM_PROFILE_SECTION_IN ("fc1553_periodSource_NT [generate trans conditions]", state1_trans_conds)
			FSM_INIT_COND (STOP)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 2, state2_enter_exec, ;, "STOP", "", "generate", "stop", "tr_2", "fc1553_periodSource_NT [generate -> stop : STOP / ]")
				FSM_CASE_TRANSIT (1, 1, state1_enter_exec, ss_packet_generate();, "PACKET_GENERATE", "ss_packet_generate()", "generate", "generate", "tr_3", "fc1553_periodSource_NT [generate -> generate : PACKET_GENERATE / ss_packet_generate()]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (2, "stop", state2_enter_exec, "fc1553_periodSource_NT [stop enter execs]")
				FSM_PROFILE_SECTION_IN ("fc1553_periodSource_NT [stop enter execs]", state2_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (5,"fc1553_periodSource_NT")


			/** state (stop) exit executives **/
			FSM_STATE_EXIT_UNFORCED (2, "stop", "fc1553_periodSource_NT [stop exit execs]")
		...
		FSM_EXIT (0,"fc1553_periodSource_NT")
		}
	}
...
void
_op_fc1553_periodSource_NT_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	/* No Diagnostic Block */
	}
...
void
_op_fc1553_periodSource_NT_terminate (OP_SIM_CONTEXT_ARG_OPT)
	{

	FIN_MT (_op_fc1553_periodSource_NT_terminate ())
...
/* local variable prs_ptr in _op_fc1553_periodSource_NT_svar function. */
#undef own_id
#undef format_str
#undef start_time
#undef stop_time
...
VosT_Obtype
_op_fc1553_periodSource_NT_init (int * init_block_ptr)
	{
	VosT_Obtype obtype = OPC_NIL;
	FIN_MT (_op_fc1553_periodSource_NT_init (init_block_ptr))

	obtype = Vos_Define_Object_Prstate ("proc state vars (fc1553_periodSource_NT)",
		sizeof (fc1553_periodSource_NT_state));
	*init_block_ptr = 0;
...
VosT_Address
_op_fc1553_periodSource_NT_alloc (VosT_Obtype obtype, int init_block)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	fc1553_periodSource_NT_state * ptr;
	FIN_MT (_op_fc1553_periodSource_NT_alloc (obtype))

	ptr = (fc1553_periodSource_NT_state *)Vos_Alloc_Object (obtype);
	if (ptr != OPC_NIL)
		{
		ptr->_op_current_block = init_block;
#if defined (OPD_ALLOW_ODB)
		ptr->_op_current_state = "fc1553_periodSource_NT [init enter execs]";
#endif
		}
...
void
_op_fc1553_periodSource_NT_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	fc1553_periodSource_NT_state		*prs_ptr;

	FIN_MT (_op_fc1553_periodSource_NT_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (fc1553_periodSource_NT_state *)gen_ptr;
