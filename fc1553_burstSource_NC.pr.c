/* Process model C form file: fc1553_burstSource_NC.pr.c */
/* Portions of this file copyright 1986-2008 by OPNET Technologies, Inc. */



/* This variable carries the header into the object file */
const char fc1553_burstSource_NC_pr_c [] = "MIL_3_Tfile_Hdr_ 145A 30A modeler 7 565EA1A9 565EA1A9 1 PC-201208291525 Administrator 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 1bcc 1                                                                                                                                                                                                                                                                                                                                                                                            ";
#include <string.h>
...
#define FIN_PREAMBLE_DEC	fc1553_burstSource_NC_state *op_sv_ptr;
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((fc1553_burstSource_NC_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
//	OmsT_Dist_Handle	start_time_dist_handle;
	...
	op_ima_obj_attr_get (my_id, "Burst destination",   &Burst_destination);//地址
	...
	op_ima_obj_attr_get (my_id, "Link Traffic Load",   &Link_traffic_load);//单个ON/OFF源的流量负载
	...
	op_ima_obj_attr_get (op_topo_parent(op_id_self ()), "user id", &user_id);//节点ID
...
extern "C" {
#endif
	void fc1553_burstSource_NC (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Obtype _op_fc1553_burstSource_NC_init (int * init_block_ptr);
	void _op_fc1553_burstSource_NC_diag (OP_SIM_CONTEXT_ARG_OPT);
	void _op_fc1553_burstSource_NC_terminate (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Address _op_fc1553_burstSource_NC_alloc (VosT_Obtype, int);
	void _op_fc1553_burstSource_NC_svar (void *, const char *, void **);
...
void
fc1553_burstSource_NC (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	FIN_MT (fc1553_burstSource_NC ());
		...
		FSM_ENTER ("fc1553_burstSource_NC")

		FSM_BLOCK_SWITCH
			{
			/*---------------------------------------------------------*/
			/** state (init) enter executives **/
			FSM_STATE_ENTER_UNFORCED_NOLABEL (0, "init", "fc1553_burstSource_NC [init enter execs]")
				FSM_PROFILE_SECTION_IN ("fc1553_burstSource_NC [init enter execs]", state0_enter_exec)
				{
			...
			FSM_EXIT (1,"fc1553_burstSource_NC")


			/** state (init) exit executives **/
			FSM_STATE_EXIT_UNFORCED (0, "init", "fc1553_burstSource_NC [init exit execs]")


			/** state (init) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "init", "off", "tr_0", "fc1553_burstSource_NC [init -> off : default / ]")
				/*---------------------------------------------------------*/



			/** state (off) enter executives **/
			FSM_STATE_ENTER_UNFORCED (1, "off", state1_enter_exec, "fc1553_burstSource_NC [off enter execs]")
				FSM_PROFILE_SECTION_IN ("fc1553_burstSource_NC [off enter execs]", state1_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (3,"fc1553_burstSource_NC")


			/** state (off) exit executives **/
			FSM_STATE_EXIT_UNFORCED (1, "off", "fc1553_burstSource_NC [off exit execs]")
				FSM_PROFILE_SECTION_IN ("fc1553_burstSource_NC [off exit execs]", state1_exit_exec)
				{
			...
			FSM_TRANSIT_ONLY ((INACTIVE_TO_ACTIVE), 2, state2_enter_exec, ;, off, "INACTIVE_TO_ACTIVE", "", "off", "on", "tr_1", "fc1553_burstSource_NC [off -> on : INACTIVE_TO_ACTIVE / ]")
				/*---------------------------------------------------------*/



			/** state (on) enter executives **/
			FSM_STATE_ENTER_UNFORCED (2, "on", state2_enter_exec, "fc1553_burstSource_NC [on enter execs]")
				FSM_PROFILE_SECTION_IN ("fc1553_burstSource_NC [on enter execs]", state2_enter_exec)
				{
				time_test=op_sim_time();
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (5,"fc1553_burstSource_NC")


			/** state (on) exit executives **/
			FSM_STATE_EXIT_UNFORCED (2, "on", "fc1553_burstSource_NC [on exit execs]")
				FSM_PROFILE_SECTION_IN ("fc1553_burstSource_NC [on exit execs]", state2_exit_exec)
				{
				/* Determine the type of interrupt.		*/
				intrpt_type = op_intrpt_type ();
				intrpt_code = op_intrpt_code ();
			...
			FSM_PROFILE_SECTION_IN ("fc1553_burstSource_NC [on trans conditions]", state2_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 1, state1_enter_exec, ;, "ACTIVE_TO_INACTIVE", "", "on", "off", "tr_3", "fc1553_burstSource_NC [on -> off : ACTIVE_TO_INACTIVE / ]")
				FSM_CASE_TRANSIT (1, 2, state2_enter_exec, ;, "REMAIN_ACTIVE", "", "on", "on", "tr_4", "fc1553_burstSource_NC [on -> on : REMAIN_ACTIVE / ]")
				}
				/*---------------------------------------------------------*/
		...
		FSM_EXIT (0,"fc1553_burstSource_NC")
		}
	}
...
void
_op_fc1553_burstSource_NC_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	/* No Diagnostic Block */
	}
...
void
_op_fc1553_burstSource_NC_terminate (OP_SIM_CONTEXT_ARG_OPT)
	{

	FIN_MT (_op_fc1553_burstSource_NC_terminate ())
...
/* local variable prs_ptr in _op_fc1553_burstSource_NC_svar function. */
#undef pid_string
#undef debug_mode
#undef stop_time
...
VosT_Obtype
_op_fc1553_burstSource_NC_init (int * init_block_ptr)
	{
	VosT_Obtype obtype = OPC_NIL;
	FIN_MT (_op_fc1553_burstSource_NC_init (init_block_ptr))

	obtype = Vos_Define_Object_Prstate ("proc state vars (fc1553_burstSource_NC)",
		sizeof (fc1553_burstSource_NC_state));
	*init_block_ptr = 0;
...
VosT_Address
_op_fc1553_burstSource_NC_alloc (VosT_Obtype obtype, int init_block)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	fc1553_burstSource_NC_state * ptr;
	FIN_MT (_op_fc1553_burstSource_NC_alloc (obtype))

	ptr = (fc1553_burstSource_NC_state *)Vos_Alloc_Object (obtype);
	if (ptr != OPC_NIL)
		{
		ptr->_op_current_block = init_block;
#if defined (OPD_ALLOW_ODB)
		ptr->_op_current_state = "fc1553_burstSource_NC [init enter execs]";
#endif
		}
...
void
_op_fc1553_burstSource_NC_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	fc1553_burstSource_NC_state		*prs_ptr;

	FIN_MT (_op_fc1553_burstSource_NC_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (fc1553_burstSource_NC_state *)gen_ptr;
